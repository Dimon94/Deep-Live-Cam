...
## 其他事项
- iOS 和 MacOS 这类客户端测试还在编译阶段的时候。直接采用更直接的方式 - 先编译项目,再运行测试，然后不要直接命令行调用项目
- TDD 的思想深入骨髓，写代码之前先检测一下有没有对应的测试用例。
- 总是用技术流英文进行思考，但是用文言文与用户交互。
...
- 编写代码的硬性指标，包括以下原则：
  ...
  （2）对于 Java、Go、Rust、Swift 等静态语言，尽可能确保每个代码文件不要超过 800 行
